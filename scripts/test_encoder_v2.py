#!/usr/bin/env python3
"""
Test MLX Encoder Model V2

This script loads and tests V2 MLX encoder models with TRUE INT8 quantization.
"""
...
def load_mlx_model_v2(model_path: Path) -> tuple:
    """
    Load MLX encoder model V2 from disk with dequantization.
...
def test_model_loading(model_path: str, verbose: bool = False):
    """Test loading the MLX model V2."""
    print(f"Testing V2 model loading from: {model_path}")
    ...
    try:
        weights, config, tokenizer = load_mlx_model_v2(model_path)
...
def test_tokenization(model_path: str, text: str = "This is a test sentence."):
    ...
    try:
        _, _, tokenizer = load_mlx_model_v2(model_path)
...
def main():
    parser = argparse.ArgumentParser(description='Test MLX encoder models V2')
    parser.add_argument(
        'model_path',
    ...
    print("="*60)
    print("MLX Encoder Model V2 Test")
    print("="*60)
